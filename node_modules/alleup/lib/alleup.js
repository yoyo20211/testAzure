...
Alleup.prototype = {
	...
	progressInfo: function(uuid){
		return this.progresses[uuid];
	},
	photoInfo: function(uuid){
		return this.photos[uuid];
	},
	upload: function(req, res, uuid, callback, _scope) {
		...
		form.addListener('progress', function(recvd, expected) {
		  ...
		  _this.progresses[uuid] = progress;
		});
		this.photos[uuid]='none';
		...
		.on('end', function() {
			...
			_this.makeVariants(new_file, function(err, file){
				_this.photos[uuid] = file;
				callback(err, file, fields, res);
			});
		});
		form.parse(req);
	},
	...
	makeVariants: function(file, callback) {
		...
		var ext = this.setExtension(file['mime']);
		new_file += ext;
