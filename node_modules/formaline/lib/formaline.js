...
var fs = require( 'fs' ),
    ...
    dirSeparator = isWindows ? "\\" : "/",
    crypto = require( 'crypto' ),
    emitter = require( 'events' ).EventEmitter,
    querystring = require( 'querystring' ),
    path = require( 'path' ),
    tmpPath = process.env.TEMP ? process.env.TEMP + "\\" : "/tmp/",
    ...
    setDebuggingLevel = function ( dstring, form ) {
        ...
        if ( filelogging || recordRequest ) {
            form.on( 'startlogging', function ( req ) {
                var fname = path.basename( form.uploadRootDir ).replace( dirSeparator, '' ),
        ...
        createUploadDir = function ( cback ) {
            ...
            path.exists( me.uploadRootDir, function ( exists ) {
                if ( exists ) {
                    ...
                    me.uploadRootDir = me.uploadRootDir + getUploadSubDirectoryName() + dirSeparator ;
                    path.exists( me.uploadRootDir, mkSubDir );
                } else {
                    // uploadRootDir doesn't exist
                    if ( me.uploadRootDir === tmpPath  ) {
                        // exception
                        jsonFatalErr.path = me.uploadRootDir;
                        ...
                        // try default root directory tmpPath 
                        ...
                        path.exists( tmpPath , function ( exists ) {
                            if ( exists ) {
                                jsonWarnErr.msg = 'switched to default root directory for uploads : "' + tmpPath  + '"';
                                me.emitEvent( 'message', jsonWarnErr, 1 );
                                me.uploadRootDir = tmpPath  + getUploadSubDirectoryName() + dirSeparator;
                                path.exists( me.uploadRootDir, mkSubDir );
                            } else {
                                // exception
                                jsonFatalErr.msg = 'default upload root directory : "'+ tmpPath  + '" does not exist ! ';
                                me.emitEvent( 'pathexception', jsonFatalErr, 0 );
                                return;
                            }
                        } );
