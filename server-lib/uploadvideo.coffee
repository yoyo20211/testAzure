...
run = ()-> 
        videoJob.get 0, (err, data, gid)->
            item = JSON.parse data
            authentication item.processId, (err,authId)->
                if err 
                    logger.log 'error', ' (Posting module) video upload id '+item.processId+' error : \n'+err
                else
                    ...
                    if item.processType is "delete"
                        ...
                        deleteVideo authId, item.processId,item._id, (err,link)->
                            if err 
                                message         = {}
                        ...
                        upload item.files.videoFile, authId, item.processId,item._id, (err,link)->
                            if err 
                                message         = {}
                                message.id      = item.processId
                                message.type    = 'video'
...
deleteVideo =  (authId, processId,itemId, callback)->
...
upload = (incommingFile,authId,processId, objectId, callback)->
